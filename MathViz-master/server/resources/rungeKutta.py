...
def post():
    ...
    try:
        ...
        for i in range(n):
            ...
            m2=float (sp.nsimplify(dy_dx.subs([(x,X[i]+h/2) , (y,Y[i]+h*(m1/2))])))
            k2=h*m2
            ...
            m3=float (sp.nsimplify(dy_dx.subs([(x,X[i]+h/2) , (y,Y[i]+h*(m2/2))])))
            k3=h*m3
        ...
        plt.annotate("$ \ Y( $"+sp.latex(int(xn))+"$)=$"+sp.latex(round(Y[-1],6)), (X[-1],Y[-1]), size=15,rotation=0)
